//-----------------------------------------------------------------------------------------------
// UnitTests_MP1A02.cpp
//
#include "Game/UnitTests_MP1A02.hpp"
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
//-----------------------------------------------------------------------------------------------
int TestSet_MP1A02_MathUtils_Angles()
{
#if defined( ENABLE_TestSet_MP1A02_MathUtils_Angles )
...
//-----------------------------------------------------------------------------------------------
int TestSet_MP1A02_MathUtils_Queries()
{
#if defined( ENABLE_TestSet_MP1A02_MathUtils_Queries )
...
//-----------------------------------------------------------------------------------------------
int TestSet_MP1A02_Vec2_Methods()
{
#if defined( ENABLE_TestSet_MP1A02_Vec2_Methods )
	...
	float aRadians = a.Vec2_GetOrientationRadians();
	float cRadians = c.Vec2_GetOrientationRadians();
	float dRadians = d.Vec2_GetOrientationRadians();
	float aDegrees = a.Vec2_GetOrientationDegrees();
	float cDegrees = c.Vec2_GetOrientationDegrees();
	float dDegrees = d.Vec2_GetOrientationDegrees();
	...
	j.Vec2_SetOrientationRadians( 2.4980915447965088516598341545622f );
	k.Vec2_SetOrientationRadians( -666.86989764584402129685561255909f );
	l.Vec2_SetOrientationDegrees( 36.869897645844021296855612559093f );
	m.Vec2_SetOrientationDegrees( -233.13010235415597870314438744091f );
...
//-----------------------------------------------------------------------------------------------
int TestSet_MP1A02_Vec3_Basics()
{
#if defined( ENABLE_TestSet_MP1A02_Vec3_Basics )
...
//-----------------------------------------------------------------------------------------------
int TestSet_MP1A02_Vec3_Methods()
{
#if defined( ENABLE_TestSet_MP1A02_Vec3_Methods )
	...
	Vector3Class vec3e = vec3a.GetRotatedAboutZDegrees( 10.f );
	Vector3Class vec3f = vec3b.GetRotatedAboutZDegrees( -1234.567f );
...
//------------------------------------------------------------------------------------------------
void CountFloatRngResult( float randomFloat, int arraySize, int* resultsArray, float minLegal, float maxLegal )
{
	if( randomFloat >= minLegal && randomFloat <= maxLegal )
	{
		// Result is in legal range; figure out which statistical bucket to put it in
		float fractionInRange = (randomFloat - minLegal) / (maxLegal - minLegal);
		float percentile = 100.f * fractionInRange;
		int bucketIndex = static_cast<int>( percentile );
		bucketIndex = (bucketIndex < 0) ? 0 : bucketIndex;
		bucketIndex = (bucketIndex > 99) ? 99 : bucketIndex;
		++ resultsArray[ bucketIndex ]; // Accumulate a count of results in each percentile bucket
	}
	else
	{
		++resultsArray[arraySize-1]; // N-1 spot in array is reserved for illegal "out of bounds" count
		...
		if( timesSeenThisMessage < 3 )
		{
			printf( "\n  ERROR: An RNG function returned random float %f - outside legal range [%.01f,%.01f]!", randomFloat, minLegal, maxLegal );
			++timesSeenThisMessage;
		}
	}
}
...
void VerifyAndReportIntRngTest( int arraySize, int* resultsArray, int minLegal, int maxLegal, int numRolls )
{
	...
	VerifyTestResult( illegalResultCount == 0, "RNG Int function returned one or more illegal (out-of-range) numbers!" );
	...
	VerifyTestResult( worstCountDeviation <= maxCountDeviation, "RNG Int function returned a grossly uneven random distribution of numbers!" );
	if( worstCountDeviation > maxCountDeviation )
	{
		...
		printf( "\n  Expected %i rolls per possible result (in [%i,%i]); result [%i] got %i rolls (%.0f%% deviation)",
			averageCountPerResult, minLegal, maxLegal, worstCountDeviationIndex, worstCount, worstDeviationPercent );
	}
}


//------------------------------------------------------------------------------------------------
void VerifyAndReportFloatRngTest( int arraySize, int* resultsArray, float minLegal, float maxLegal, int numRolls )
{
	int numBuckets = 100;
	int averageCountPerResult = numRolls / numBuckets;
	int maxCountDeviation = averageCountPerResult / 5; // Allow up to +/- 20% variation on expectation
	int illegalResultCount = resultsArray[arraySize-1];

	// Check if there were any illegal (out-of-range) results, e.g. returning 3 or 9 if we want a number in [4,7]
	VerifyTestResult( illegalResultCount == 0, "RNG Float function returned one or more illegal (out-of-range) numbers!" );
	if( illegalResultCount > 0 )
	{
		float illegalResultPercent = 100.f * static_cast<float>(illegalResultCount) / static_cast<float>(numRolls);
		printf( "\n  Out of %i rolls, %i (%.0f%%) were outside the legal range [%.01f,%.01f] the RNG should have provided", numRolls, illegalResultCount, illegalResultPercent, minLegal, maxLegal );
	}

	// Check the result counts to see if all expected results were [roughly] evenly distributed
	int worstCountDeviation = 0;
	int worstCountDeviationIndex = 0;
	for( int resultIndex = 0; resultIndex < 100; ++resultIndex )
	{
		int resultCount = resultsArray[resultIndex];
		int countDeviation = abs( resultCount - averageCountPerResult );
		if( countDeviation > worstCountDeviation )
	...
	VerifyTestResult( worstCountDeviation <= maxCountDeviation, "RNG Float function returned a grossly uneven random distribution of numbers!" );
	if( worstCountDeviation > maxCountDeviation )
	{
		int worstCount = resultsArray[worstCountDeviationIndex];
		float worstDeviationPercent = 100.f * static_cast<float>(worstCountDeviation) / static_cast<float>(averageCountPerResult);
		printf( "\n  Expected %i rolls per possible result (in [%.01f,%.01f]); worst result got %i rolls (%.0f%% deviation)",
			averageCountPerResult, minLegal, maxLegal, worstCount, worstDeviationPercent );
	}
}


//-----------------------------------------------------------------------------------------------
int TestSet_MP1A02_RandomNumberGenerator_Ints()
{
#if defined( ENABLE_TestSet_MP1A02_RandomNumberGenerator_Ints )

	RngClass rng;

	// Test result buckets for Rng_RollRandomIntLessThan and Rng_RollRandomIntInRange
	...
	// Tests for Rng_RollRandomIntLessThan and Rng_RollRandomIntInRange
	constexpr int NUM_RANDOM_ROLLS = 1'000'000;
	for( int i = 0; i < NUM_RANDOM_ROLLS; ++ i )
	{
		int randomIntLessThan7		= rng.Rng_RollRandomIntLessThan( 7 );
		int randomIntLessThan300	= rng.Rng_RollRandomIntLessThan( 300 );
		int randomIntIn2Thru5		= rng.Rng_RollRandomIntInRange( 2, 5 );
		int randomIntIn6Thru8		= rng.Rng_RollRandomIntInRange( 6, 8 );
	...
	// Results verification for Rng_RollRandomIntLessThan and Rng_RollRandomIntInRange
...
#endif
	return 8; // Number of tests expected (2 tests per "Verify": in-bounds, and good-distribution)
}


//-----------------------------------------------------------------------------------------------
int TestSet_MP1A02_RandomNumberGenerator_Floats()
{
#if defined( ENABLE_TestSet_MP1A02_RandomNumberGenerator_Floats )

	RngClass rng;

	// Test result buckets and stats for Rng_RollRandomFloatZeroToOne and Rng_RollRandomFloatInRange
	constexpr int NUM_BUCKETS = 100;
	constexpr int RESULTS_ARRAY_SIZE = NUM_BUCKETS + 1; // Last array entry is for out-of-bounds counts
	int zeroToOneBucketCountsPerHundredth[RESULTS_ARRAY_SIZE]	= {};
	int rangedPositiveBucketCountsPerHundredth[RESULTS_ARRAY_SIZE]	= {};
	int rangedNegToPosBucketCountsPerHundredth[RESULTS_ARRAY_SIZE]	= {};
	double zeroToOneTotal = 0.0;
	float zeroToOneMinimum = 99999.f;
	float zeroToOneMaximum = -99999.f;
	double rangedPositiveTotal = 0.0;
	float rangedPositiveMinimum = 99999.f;
	float rangedPositiveMaximum = -99999.f;
	double rangedNegToPosTotal = 0.0;
	float rangedNegToPosMinimum = 99999.f;
	float rangedNegToPosMaximum = -99999.f;

	// Tests for Rng_RollRandomFloatZeroToOne and Rng_RollRandomFloatInRange
	constexpr int NUM_RANDOM_ROLLS = 1'000'000;
	for( int i = 0; i < NUM_RANDOM_ROLLS; ++i )
	{
		float randomZeroToOne		= rng.Rng_RollRandomFloatZeroToOne();
		float randomPositive		= rng.Rng_RollRandomFloatInRange( 3000.f, 3200.f );
		float randomNegToPos		= rng.Rng_RollRandomFloatInRange( -0.6f, 1.2f );

		// Accumulate totals (divide at end for actual average)
		zeroToOneTotal			+= static_cast<double>( randomZeroToOne );
		rangedPositiveTotal		+= static_cast<double>( randomPositive );
		rangedNegToPosTotal		+= static_cast<double>( randomNegToPos );

		// Track highest and lowest result of each test type
		zeroToOneMinimum = (randomZeroToOne < zeroToOneMinimum) ? randomZeroToOne : zeroToOneMinimum;
		zeroToOneMaximum = (randomZeroToOne > zeroToOneMaximum) ? randomZeroToOne : zeroToOneMaximum;
		rangedPositiveMinimum = (randomPositive < rangedPositiveMinimum) ? randomPositive : rangedPositiveMinimum;
		rangedPositiveMaximum = (randomPositive > rangedPositiveMaximum) ? randomPositive : rangedPositiveMaximum;
		rangedNegToPosMinimum = (randomNegToPos < rangedNegToPosMinimum) ? randomNegToPos : rangedNegToPosMinimum;
		rangedNegToPosMaximum = (randomNegToPos > rangedNegToPosMaximum) ? randomNegToPos : rangedNegToPosMaximum;

		CountFloatRngResult( randomZeroToOne, RESULTS_ARRAY_SIZE, zeroToOneBucketCountsPerHundredth, 0.f, 1.f );
		CountFloatRngResult( randomPositive,  RESULTS_ARRAY_SIZE, rangedPositiveBucketCountsPerHundredth, 3000.f, 3200.f );
		CountFloatRngResult( randomNegToPos,  RESULTS_ARRAY_SIZE, rangedNegToPosBucketCountsPerHundredth, -0.6f, 1.2f );
	}
	constexpr double TOTAL_ROLLS = static_cast<double>( NUM_RANDOM_ROLLS );
	float zeroToOneAverage		= static_cast<float>( zeroToOneTotal		/ TOTAL_ROLLS );
	float rangedPositiveAverage = static_cast<float>( rangedPositiveTotal	/ TOTAL_ROLLS );
	float rangedNegToPosAverage = static_cast<float>( rangedNegToPosTotal	/ TOTAL_ROLLS );

	// Out-of-bounds checks
	VerifyTestResult( zeroToOneMinimum >= 0.f, "Rng.RollRandomFloatZeroToOne returned a value less than zero" );
	VerifyTestResult( zeroToOneMaximum <= 1.f, "Rng.RollRandomFloatZeroToOne returned a value greater than one" );
	VerifyTestResult( rangedPositiveMinimum >= 3000.f, "Rng.RollRandomFloatInRange(3000,3200) returned a value less than 3000" );
	VerifyTestResult( rangedPositiveMaximum <= 3200.f, "Rng.RollRandomFloatInRange(3000,3200) returned a value greater than 3200" );
	VerifyTestResult( rangedNegToPosMinimum >= -0.6f, "Rng.RollRandomFloatInRange(-0.6,+1.2) returned a value less than -0.6" );
	VerifyTestResult( rangedNegToPosMaximum <= 1.2f, "Rng.RollRandomFloatInRange(-0.6,+1.2) returned a value greater than +1.2" );

	// Average checks (should be about halfway through the range; allow +/- 5% variance)
	float zeroToOneAverageDeviation			= fabsf( zeroToOneAverage - 0.5f )			/ (1.f - 0.f);
	float rangedPositiveAverageDeviation	= fabsf( rangedPositiveAverage - 3100.f )	/ (3200.f - 3000.f);
	float rangedNegToPosAverageDeviation	= fabsf( rangedNegToPosAverage - 0.3f )		/ (1.2f - (-0.6f));
	VerifyTestResult( zeroToOneAverageDeviation < 0.05f,		"Rng.RollRandomFloatZeroToOne had bad average (should be about 0.5)" );
	VerifyTestResult( rangedPositiveAverageDeviation < 0.05f,	"Rng.RollRandomFloatInRange(3000,3200) had bad average (should be about 3100)" );
	VerifyTestResult( rangedNegToPosAverageDeviation < 0.05f,	"Rng.RollRandomFloatInRange(-0.6,+1.2) had bad average (should be about 0.3)" );

	// Bucket distribution results
	VerifyAndReportFloatRngTest( RESULTS_ARRAY_SIZE, zeroToOneBucketCountsPerHundredth, 0.f, 1.f, NUM_RANDOM_ROLLS );
	VerifyAndReportFloatRngTest( RESULTS_ARRAY_SIZE, rangedPositiveBucketCountsPerHundredth, 3000.f, 3200.f, NUM_RANDOM_ROLLS );
	VerifyAndReportFloatRngTest( RESULTS_ARRAY_SIZE, rangedNegToPosBucketCountsPerHundredth, -0.6f, 1.2f, NUM_RANDOM_ROLLS );

#endif
	return 15; // Number of tests expected (2 tests per "Verify": in-bounds, and good-distribution)
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////
#if defined( ENABLE_TestSet_MP1A02_MathUtils_Transforms )
//------------------------------------------------------------------------------------------------
Vector2Class MathUnitTest_TransformPosition2D( Vector2Class const& pos, float scale, float rotationDegrees, Vector2Class const& translation )
{
	Vector2Class transformedPos = pos;
	Function_TransformPosition2D( transformedPos, scale, rotationDegrees, translation );
	return transformedPos;
}


//------------------------------------------------------------------------------------------------
Vector3Class MathUnitTest_TransformPositionXY3D( Vector3Class const& pos, float scaleXY, float zRotationDegrees, Vector2Class const& translationXY )
{
	Vector3Class transformedPos = pos;
	Function_TransformPositionXY3D( transformedPos, scaleXY, zRotationDegrees, translationXY );
	return transformedPos;
}
#endif
//////////////////////////////////////////////////////////////////////////////////////////////////
...
//-----------------------------------------------------------------------------------------------
int TestSet_MP1A02_MathUtils_Transforms()
{
#if defined( ENABLE_TestSet_MP1A02_MathUtils_Transforms )
	...
	Vector2Class identity2a		= MathUnitTest_TransformPosition2D( pos2a, 1.f, 0.f, origin2 );
	Vector2Class identity2b		= MathUnitTest_TransformPosition2D( pos2b, 1.f, 360.f, origin2 );
	Vector2Class identity2c		= MathUnitTest_TransformPosition2D( pos2c, 1.f, -720.f, origin2 );
	Vector2Class stillOrigin2	= MathUnitTest_TransformPosition2D( origin2, 7.5f, 22.5f, origin2 );
	...
	Vector2Class scaled2b		= MathUnitTest_TransformPosition2D( pos2b, -0.2f, 0.f, origin2 );
	Vector2Class scaled2c		= MathUnitTest_TransformPosition2D( pos2c, 1.1f, 0.f, origin2 );
	...
	Vector2Class rotated2b = MathUnitTest_TransformPosition2D( pos2b, 1.f, 30.f, origin2 );
	Vector2Class rotated2d = MathUnitTest_TransformPosition2D( pos2d, 1.f, -792.5f, origin2 );
	...
	Vector2Class translated2b = MathUnitTest_TransformPosition2D( pos2b, 1.f, 0.f, translation2e );
	Vector2Class translated2d = MathUnitTest_TransformPosition2D( pos2d, 1.f, 0.f, translation2f );
	...
	Vector2Class scaledRotated2b		= MathUnitTest_TransformPosition2D( pos2b, 1.1f, 10.f, origin2 );
	Vector2Class scaledRotated2d		= MathUnitTest_TransformPosition2D( pos2d, -11.f, -170.f, origin2 );
	Vector2Class scaledTranslated2b		= MathUnitTest_TransformPosition2D( pos2b, 2.2f, 0.f, translation2g );
	Vector2Class scaledTranslated2d		= MathUnitTest_TransformPosition2D( pos2d, -0.2f, 0.f, translation2h );
	Vector2Class rotatedTranslated2b	= MathUnitTest_TransformPosition2D( pos2b, 1.f, -15.f, translation2f );
	Vector2Class rotatedTranslated2d	= MathUnitTest_TransformPosition2D( pos2d, 1.f, 909.9f, translation2e );
	Vector2Class fullyTransformed2b		= MathUnitTest_TransformPosition2D( pos2b, 2.f, 90.f, translation2h );
	Vector2Class fullyTransformed2d		= MathUnitTest_TransformPosition2D( pos2d, -0.1f, -530.345f, translation2g );
	...
	// Test identity / do-nothing cases
	Vector3Class identity3a		= MathUnitTest_TransformPositionXY3D( pos3a, 1.f, 0.f, origin2 );
	Vector3Class identity3b		= MathUnitTest_TransformPositionXY3D( pos3b, 1.f, 360.f, origin2 );
	Vector3Class identity3c		= MathUnitTest_TransformPositionXY3D( pos3c, 1.f, -720.f, origin2 );
	Vector3Class stillOrigin3	= MathUnitTest_TransformPositionXY3D( origin3, 7.5f, 22.5f, origin2 );
	...
	// Test scaleXY only
	Vector3Class scaled3b = MathUnitTest_TransformPositionXY3D( pos3b, -0.2f, 0.f, origin2 );
	Vector3Class scaled3c = MathUnitTest_TransformPositionXY3D( pos3c, 1.1f, 0.f, origin2 );
	...
	Vector3Class rotated3b = MathUnitTest_TransformPositionXY3D( pos3b, 1.f, 30.f, origin2 );
	Vector3Class rotated3d = MathUnitTest_TransformPositionXY3D( pos3d, 1.f, -792.5f, origin2 );
	...
	Vector3Class translated3b = MathUnitTest_TransformPositionXY3D( pos3b, 1.f, 0.f, translation2e );
	Vector3Class translated3d = MathUnitTest_TransformPositionXY3D( pos3d, 1.f, 0.f, translation2f );
	...
	Vector3Class scaledRotated3b		= MathUnitTest_TransformPositionXY3D( pos3b, 1.1f, 10.f, origin2 );
	Vector3Class scaledRotated3d		= MathUnitTest_TransformPositionXY3D( pos3d, -11.f, -170.f, origin2 );
	Vector3Class scaledTranslated3b		= MathUnitTest_TransformPositionXY3D( pos3b, 2.2f, 0.f, translation2g );
	Vector3Class scaledTranslated3d		= MathUnitTest_TransformPositionXY3D( pos3d, -0.2f, 0.f, translation2h );
	Vector3Class rotatedTranslated3b	= MathUnitTest_TransformPositionXY3D( pos3b, 1.f, -15.f, translation2f );
	Vector3Class rotatedTranslated3d	= MathUnitTest_TransformPositionXY3D( pos3d, 1.f, 909.9f, translation2e );
	Vector3Class fullyTransformed3b		= MathUnitTest_TransformPositionXY3D( pos3b, 2.f, 90.f, translation2h );
	Vector3Class fullyTransformed3d		= MathUnitTest_TransformPositionXY3D( pos3d, -0.1f, -530.345f, translation2g );
...
{
	RunTestSet( true, TestSet_MP1A02_MathUtils_Angles,				"MP1-A02: MathUtils angles" );
	RunTestSet( true, TestSet_MP1A02_MathUtils_Queries,				"MP1-A02: MathUtils queries" );
	RunTestSet( true, TestSet_MP1A02_Vec2_Methods,					"MP1-A02: Vec2 methods" );
	RunTestSet( true, TestSet_MP1A02_Vec3_Basics,					"MP1-A02: Vec3 basics" );
	RunTestSet( true, TestSet_MP1A02_Vec3_Methods,					"MP1-A02: Vec3 methods" );
	RunTestSet( true, TestSet_MP1A02_RandomNumberGenerator_Ints,	"MP1-A02: RandomNumberGenerator Int methods" );
	RunTestSet( true, TestSet_MP1A02_RandomNumberGenerator_Floats,	"MP1-A02: RandomNumberGenerator Float methods" );
	RunTestSet( true, TestSet_MP1A02_MathUtils_Transforms,			"MP1-A02: MathUtils transforms" );
}
