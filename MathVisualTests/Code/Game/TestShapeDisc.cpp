...
void TestShapeDisc::Render() const
{
	...
	g_engine->m_render->DrawVertexArray( ( int )m_myVerts.size(), m_myVerts.data() );
...
{
	for (int i = 0; i <  static_cast<int>(m_myVerts.size()); ++i)
	{
		m_myVerts[i].m_color = newColor;
	}
}